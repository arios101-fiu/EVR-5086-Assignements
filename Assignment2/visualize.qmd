...
track_storm <- function(dat, storm_id, zoom = 4,
  ...
  m <- leaflet(options = leafletOptions(worldCopyJump = TRUE)) |>
    addTiles() |>
    setView(lng = initial_location[2], lat = initial_location[1], zoom = zoom) |>
    addPolylines(
      lng = storm$lon,
      lat = storm$lat,
      color = "blue",
...
```{r}
#| eval: !expr knitr::is_latex_output()   # show widget only in HTML
m <- track_storm(hurdat_ar, storm_id = "AL092021")
m
```

```{r}
#| fig-cap: "Leaflet storm track (static screenshot for PDF)."
#| eval: !expr knitr::is_html_output()    # only in PDF
m <- track_storm(hurdat_ar, storm_id = "AL092021")
